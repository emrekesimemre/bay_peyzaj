...
export default function IletisimClient({ locale }: { locale: Locale }) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const win = window.open(`${SITE.social.whatsapp}?text=${encodeURIComponent(text)}`, "_blank");
    if (win) setState("success");
  }

  const infoItems = [
    { label: t("labelPhone"), value: SITE.phoneDisplay, href: `tel:+${SITE.phoneRaw}` },
    { label: t("labelEmail"), value: SITE.email, href: `mailto:${SITE.email}` },
    { label: t("labelAddress"), value: `${SITE.address.street}\n${SITE.address.city}, ${SITE.address.country}`, href: SITE.address.mapsUrl },
    ...
    <div className="pt-32 pb-24 px-6 md:px-16 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24">
        ...
        <div className="space-y-10">
          {infoItems.map((item) => (
            <div key={item.label}>
              <p className="font-sans text-[10px] tracking-[0.25em] uppercase text-antrasit/40 mb-2">{item.label}</p>
              {item.href ? (
                ...
                <p className="font-sans text-base text-antrasit/70 whitespace-pre-line leading-relaxed">{item.value}</p>
              )}
            </div>
          ))}

          <div>
            <p className="font-sans text-[10px] tracking-[0.25em] uppercase text-antrasit/40 mb-4">{t("labelSocial")}</p>
          ...
          <div className="pt-6 border-t border-antrasit/10">
            <p className="font-sans text-[10px] tracking-[0.25em] uppercase text-antrasit/40 mb-3">{t("labelServiceArea")}</p>
            <p className="font-sans text-sm text-antrasit/60 leading-relaxed">{t("serviceAreaText")}</p>
          </div>
        </div>
        ...
        {state === "success" ? (
          <div className="flex flex-col justify-center gap-6 py-12">
            ...
            <h2 className="font-serif text-3xl font-semibold text-antrasit">{t("successTitle")}</h2>
            <p className="font-sans text-base text-antrasit/60 leading-relaxed max-w-sm">{t("successText")}</p>
            <button
              onClick={() => { setName(""); setPhone(""); setService(""); setMessage(""); setKvkk(false); setState("idle"); }}
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="group">
              <label htmlFor="contact-name" className="block font-sans text-xs tracking-[0.15em] uppercase text-antrasit/50 mb-2 group-focus-within:text-servi-yesili transition-colors duration-200">
                {t("formName")}
              </label>
              <input id="contact-name" type="text" required autoComplete="name" value={name}
                onChange={(e) => setName(e.target.value)} placeholder={t("formNamePlaceholder")}
            ...
            <div className="group">
              <label htmlFor="contact-phone" className="block font-sans text-xs tracking-[0.15em] uppercase text-antrasit/50 mb-2 group-focus-within:text-servi-yesili transition-colors duration-200">
                {t("formPhone")}
              </label>
              <input id="contact-phone" type="tel" required autoComplete="tel" value={phone}
                onChange={(e) => setPhone(e.target.value)} placeholder={t("formPhonePlaceholder")}
                pattern="[0-9\s\(\)\+\-]{7,15}" title={t("formPhoneTitle")}
                className="w-full bg-transparent border-0 border-b border-antrasit/20 focus:border-servi-yesili pb-3 font-sans text-base text-antrasit placeholder:text-antrasit/25 outline-none transition-colors duration-300" />
            </div>

            <div className="group">
              <label htmlFor="contact-service" className="block font-sans text-xs tracking-[0.15em] uppercase text-antrasit/50 mb-2 group-focus-within:text-servi-yesili transition-colors duration-200">
                {t("formService")}
              </label>
              <select id="contact-service" value={service} onChange={(e) => setService(e.target.value)}
                ...
                {serviceLabels.map((s) => <option key={s} value={s}>{s}</option>)}
              </select>
            </div>

            <div className="group">
              <label htmlFor="contact-message" className="block font-sans text-xs tracking-[0.15em] uppercase text-antrasit/50 mb-2 group-focus-within:text-servi-yesili transition-colors duration-200">
                {t("formMessage")}
              </label>
            ...
            <div className="flex items-start gap-3">
              <input id="contact-kvkk" type="checkbox" required checked={kvkk} onChange={(e) => setKvkk(e.target.checked)}
                className="mt-0.5 h-4 w-4 shrink-0 accent-servi-yesili cursor-pointer" />
              <label htmlFor="contact-kvkk" className="font-sans text-xs text-antrasit/50 leading-relaxed cursor-pointer">
                {t("kvkkBefore")}
            ...
            <button type="submit" disabled={!kvkk}
