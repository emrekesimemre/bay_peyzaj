        ...
        >
          {faqItems.map((item, i) => (
            <div key={i} className="border-b border-antrasit/10">
              ...
              >
                <span className="font-serif text-lg md:text-xl text-antrasit leading-snug">{item.question}</span>
                <span className={`mt-1 shrink-0 text-servi-yesili transition-transform duration-300 ${openIndex === i ? "rotate-45" : ""}`} aria-hidden="true">
              ...
              <AnimatePresence initial={false}>
                {openIndex === i && (
                  ...
                  >
                    <p className="font-sans text-base text-antrasit/60 leading-relaxed pb-6">{item.answer}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
