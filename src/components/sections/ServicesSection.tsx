        ...
        <motion.div ref={ref} variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"}>
          {services.map((service) => (
            <motion.div key={service.number} variants={itemVariants}>
              <motion.div variants={dividerVariants} className="h-px bg-antrasit/10 origin-left" style={{ transformOrigin: "left" }} />
              <div className="py-8 flex gap-6 items-start">
                <span className="font-sans text-xs tracking-widest text-servi-yesili pt-1 shrink-0 select-none">{service.number}</span>
                <div>
                  <h3 className="font-serif text-xl md:text-2xl text-antrasit mb-2">{service.title}</h3>
                  <p className="font-sans text-sm md:text-base text-antrasit/50 leading-relaxed">{service.description}</p>
                </div>
              </div>
            </motion.div>
          ))}
